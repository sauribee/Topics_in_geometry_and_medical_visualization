...
def main():
    ...
    parser.add_argument(
        "--example",
        choices=["ellipse", "custom"],
        default="ellipse",
        help="Example to run",
    )
    parser.add_argument(
        "--points", nargs="+", help="Custom points in format x1,y1 x2,y2 ..."
    )
    parser.add_argument(
        "--show_derivatives",
        action="store_true",
    ...
    if args.example == "ellipse":
        ...
        curve = ellipse_example(
            save_only=args.save_only, output_dir=output_dir, timestamp=timestamp
        )
