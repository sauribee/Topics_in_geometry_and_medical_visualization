...
def rotator_cuff_example(
    ...
    spline = QuadraticBSpline(
        ...
        monotone_parameterization=(
            not sort_angular
        ),  # If points are already sorted, skip this
    )
    ...
    # Add detailed title
    title_text = (
        "Interpolación de contorno de manguito rotador con B-spline cúbico mejorado"
    )
    if sort_angular:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="B-Spline Curve for Medical Image Contours"
    )
    parser.add_argument(
        "--points_file",
        default="ptos.mat",
    ...
    parser.add_argument(
        "--output_dir", default=None, help="Directory to save the output images"
    )
    parser.add_argument(
        "--smoothing",
        type=float,
