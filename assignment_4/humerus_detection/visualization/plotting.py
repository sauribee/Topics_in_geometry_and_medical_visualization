...
def visualize_results(
    ...
    if len(contour) > 0:
        ax.plot(contour[:, 1], contour[:, 0], 'r-', linewidth=1, label='Detected contour')
...
def save_results(figure: plt.Figure, output_path: str, base_name: str, suffix: str = "result") -> None:
    """
...
def create_composite_figure(
    ...
    if show_segmentation:
        ...
        if x_spline is not None and y_spline is not None:
            ...
            rr, cc = polygon(y_spline.astype(int), x_spline.astype(int), pixel_array.shape)
...
def visualize_3d_contours(contours: List[np.ndarray], z_positions: List[float], 
                        pixel_spacing: Tuple[float, float] = (1.0, 1.0)) -> plt.Figure:
    """
