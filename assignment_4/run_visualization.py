    ...
    for i, file in enumerate(files):
        ...
        if len(coords) > 10:  # If there are enough points to form a contour
            ...
            if contours:
                ...
                reduced_contour = last_center + (last_contour - last_center) * reduction_factor
...
def generate_3d_model(contours, z_spacing, pixel_spacing, output_path):
    ...
    # Create GIF
    imageio.mimsave(os.path.join(output_path, '3d_model_rotation.gif'), frames, duration=0.2)
    ...
    print(f"Rotation GIF saved to: {os.path.join(output_path, '3d_model_rotation.gif')}")
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Generate 3D visualization of the humerus from detected contours.")
