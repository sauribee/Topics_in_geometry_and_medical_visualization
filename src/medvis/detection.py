...
def detect_humerus_circle(
    ...
    edges = feature.canny(
        smoothed_image, sigma=2.0, low_threshold=0.1, high_threshold=0.2
    )
    ...
    for i, (cx_i, cy_i, radius_i, acc_i) in enumerate(zip(cx, cy, radii, accums)):
        ...
        distance = np.sqrt(
            (cx_i - image_center[0]) ** 2 + (cy_i - image_center[1]) ** 2
        )
        ...
        rr, cc = circle_perimeter(
            int(cy_i), int(cx_i), int(radius_i), shape=pixel_array.shape
        )

        if len(rr) > 0 and len(cc) > 0:
            circle_mask[rr, cc] = True
            # Dilate to fill the circle
            circle_mask = binary_dilation(
                circle_mask, structure=morphology_disk(radius_i // 2)
            )
...
def generate_circular_contour(
    center_x: float, center_y: float, radius: float, num_points: int = 100
) -> np.ndarray:
    """
...
def generate_terminal_contour(
    previous_contour: np.ndarray, reduction_factor: float
) -> np.ndarray:
    """
    ...
    if reduction_factor < 0.3:
        ...
        radius = (
            np.max(np.sqrt(np.sum((previous_contour - center) ** 2, axis=1)))
            * reduction_factor
        )
...
def detect_advanced_humerus_contour(
    ...
    if file_name in final_slices:
        ...
        central_mask = image_processing.create_circular_mask(
            center_y, center_x, analysis_radius, pixel_array.shape
        )
    ...
    is_problematic_image = file_name in [
        "I06",
        "I07",
        "I12",
        "I13",
    ...
    for idx, cont in enumerate(contours):
        print(
            f"Contour {idx}: Points={len(cont)}, Area={geometry.calculate_contour_area(cont):.2f}"
        )
    ...
    if len(contours) == 0 and file_name in final_slices:
        print(
            f"No contours detected in {file_name}, the humerus may have already disappeared"
        )
        return np.array([])
    ...
    if previous_contour is not None and is_problematic_image:
        ...
        if (
            ...
            and geometry.calculate_contour_area(best_contour) > 100
        ):
        ...
        for contour in contours:
            ...
            if area > 100 and circularity > best_circularity:
                ...
                if (
                    np.linalg.norm([center[1] - center_x, center[0] - center_y])
                    < 0.7 * radius
                ):
    ...
    if file_name in final_slices and best_contour is not None:
        ...
        if contour_area < 200:  # Higher minimum area threshold for final slices
            print(
                f"Contour too small in {file_name} (area={contour_area:.2f}), possible noise"
            )
            return np.array([])
    ...
    if best_contour is not None and len(best_contour) > 10:
        try:
            ...
            if snake_area < 50 and previous_contour is not None:
                print(
                    f"Snake collapsed for {file_name} (area={snake_area:.2f}). Using previous contour."
                )
                return previous_contour
            ...
            if previous_contour is not None:
                ...
                if np.linalg.norm(displacement) > max_disp:
                    displacement = displacement * (
                        max_disp / np.linalg.norm(displacement)
                    )
            ...
            if circularity < 0.7:
                print(
                    f"Irregular contour detected in {file_name} (circularity={circularity:.2f}). Applying additional smoothing."
                )
    ...
    if best_contour is None or geometry.calculate_contour_area(best_contour) < 50:
        ...
        if file_name in final_slices:
            print(
                f"No valid contour detected in {file_name}, the humerus may have already disappeared"
            )
            return np.array([])
        else:
            print(
                f"Using previous contour or circle for {file_name} due to insufficient detection."
            )
            return (
                previous_contour
                if previous_contour is not None
                else generate_circular_contour(center_x, center_y, radius)
            )

    return best_contour


def select_humerus_contour(
    contours: List[np.ndarray], pixel_array: np.ndarray
) -> np.ndarray:
    """
    ...
    if best_contour is None or len(best_contour) == 0:
        ...
        best_contour = generate_circular_contour(
            image_center[0], image_center[1], estimated_radius
        )
