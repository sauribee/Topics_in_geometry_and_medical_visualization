...
def create_circular_mask(
    center_y: int, center_x: int, radius: int, shape: Tuple[int, int], fill: bool = True
) -> np.ndarray:
    """
...
def threshold_roi(
    image: np.ndarray, roi_mask: np.ndarray, block_size: int = 31, offset: float = -0.1
) -> np.ndarray:
    """
    ...
    local_threshold = threshold_local(
        roi_image, block_size=block_size, method="gaussian", offset=offset
    )
