...
def read_dicom(
    file_path: str,
) -> Tuple[Optional[pydicom.dataset.FileDataset], Optional[np.ndarray]]:
    """
...
def load_slice_spacing(dicom_directory: str) -> Tuple[float, Tuple[float, float]]:
    ...
    if len(files) > 1:
        try:
            ...
            elif hasattr(ds1, "ImagePositionPatient") and hasattr(
                ds2, "ImagePositionPatient"
            ):
                z_spacing = abs(
                    ds2.ImagePositionPatient[2] - ds1.ImagePositionPatient[2]
                )
            else:
