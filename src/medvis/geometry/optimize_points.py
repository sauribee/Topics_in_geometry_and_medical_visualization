...
def create_original_curve(a=4, b=5):
    ...
    points = np.array(
        [(a * np.cos(angle), b * np.sin(angle)) for angle in original_angles]
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Apply optimized points to ellipse spline"
    )
    parser.add_argument(
    ...
    print("Original spline vs. Optimized spline:")
    ...
    # Save image
    _ = save_plot(optimized_curve, args.a, args.b, output_dir, filename)

    print("\nOptimized spline image created!")
    print("You can use this image for your applied geometry assignment.")
