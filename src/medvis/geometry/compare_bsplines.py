...
def generate_comparison_grid(
    points_file="ptos.mat", output_dir=None, sort_angular=True
):
    """
...
def generate_animation_frames(
    points_file="ptos.mat", output_dir=None, degree=3, smoothing=0.0, sort_angular=True
):
    """
    ...
    for i in range(n_frames):
        ...
        ax.set_title(
            f"Contour with Degree {degree} B-spline (View {i+1}/{n_frames})",
            fontsize=14,
        )
    ...
    print(
        f"convert -delay 10 -loop 0 {animation_dir}/frame_*.png {output_dir}/animation_degree{degree}_smooth{int(smoothing)}_{timestamp}.gif"
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="B-spline Comparison for Medical Contours"
    )
    parser.add_argument(
        "--points_file",
        default="ptos.mat",
    ...
    parser.add_argument(
        "--mode",
        choices=["grid", "animation"],
        default="grid",
        help="Operation mode: grid (comparison grid) or animation (frames for animation)",
    )
    parser.add_argument(
        "--degree", type=int, default=3, help="B-spline degree for animation mode"
    )
    parser.add_argument(
        "--smoothing",
        type=float,
