    ...
    def __call__(self, t_new):
        ...
        for i, t_val in enumerate(t_new):
            if t_val < self.t[0] or t_val > self.t[-1]:
                raise ValueError(
                    f"Parameter value {t_val} is outside the interpolation range [{self.t[0]}, {self.t[-1]}]"
                )
...
class Spline2D:
    ...
    def __init__(self, points, t=None):
        ...
        if t is None:
            ...
            if len(t) != len(points):
                raise ValueError(
                    "Number of parameter values must match number of points"
                )
...
def ellipse_example(save_only=False, output_dir=None, timestamp=None):
    ...
    ax.set_title(
        f"Quadratic Spline Curve Interpolating Points on Ellipse (a={a}, b={b})"
    )
    ax.legend()
