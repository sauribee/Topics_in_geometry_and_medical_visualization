...
def generate_curve_with_points(points_angles, a=4, b=5):
    ...
    points = np.array([(a*np.cos(angle), b*np.sin(angle)) for angle in points_angles])
...
def random_search(num_iterations=1000, num_points=5, a=4, b=5, seed=None):
    ...
    for i in range(num_iterations):
        ...
        if error < best_error:
            ...
            if (i+1) % 100 == 0 or i == 0:
                print(f"Iteration {i+1}/{num_iterations}: New best error = {best_error:.6f}")
...
def save_comparison_plot(best_curve, original_curve, a, b, output_dir, timestamp=None):
    ...
    ax1.plot(original_points[:, 0], original_points[:, 1], 'b-', label='Original Spline')
    
    # Show original control points
    ax1.plot(original_curve.points[:, 0], original_curve.points[:, 1], 'ro', label='Orig. Control Points')
    ...
    ax2.plot(best_curve.points[:, 0], best_curve.points[:, 1], 'mo', label='Opt. Control Points')
...
def main():
    ...
    parser.add_argument('--points', type=int, default=5,
                        help='Number of control points for the spline')
    ...
    print(f"Searching for the best configuration of {args.points} points to approximate an ellipse...")
    ...
    # Save comparison
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    filepath = save_comparison_plot(best_curve, original_curve, args.a, args.b, output_dir, timestamp)
