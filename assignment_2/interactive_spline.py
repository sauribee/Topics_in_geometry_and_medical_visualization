...
class InteractiveSpline:
    def __init__(self):
        ...
        self.points = np.array([(self.a*np.cos(ui), self.b*np.sin(ui)) for ui in self.u])
    ...
    def update_plot(self):
        ...
        ellipse_points = np.column_stack((self.a * np.cos(theta_samples), self.b * np.sin(theta_samples)))
        
        # Map theta values to t parameters (approximate)
        # First normalize theta between 0 and 1
        normalized_theta = (theta_samples - theta_samples[0]) / (theta_samples[-1] - theta_samples[0])
    ...
    def on_motion(self, event):
        ...
        if (self.selected_point == 0 or new_t > self.u[self.selected_point-1]) and \
           (self.selected_point == len(self.u)-1 or new_t < self.u[self.selected_point+1]):
...
def main():
    ...
    interactive_spline = InteractiveSpline()
    plt.show()
